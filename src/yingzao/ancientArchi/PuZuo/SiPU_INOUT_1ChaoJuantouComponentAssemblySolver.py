...
class SiPU_INOUT_1ChaoJuantouComponentAssemblySolver(object):
    ...
    def step1_read_db(self):
        self.Log.append(u"[Step1] Read DBJsonReader (PuZuo / type_code=SiPU_INOUT_1ChaoJuantou).")

        reader = DBJsonReader(
            db_path=self.DBPath,
            table="PuZuo",
            key_field="type_code",
            key_value="SiPU_INOUT_1ChaoJuantou",
            field="params_json",
            json_path=None,
            export_all=True,
            ghenv=self.ghenv
    ...
    def step3_nidaogong_huagong_and_align(self):
        ...
        # ---- 3.3 HuaGong ----
        try:
            self.HuaGong_solver = HuaGong_4PU_INOUT_1ChaoJuantou_Solver(self.DBPath, base_point, self.Refresh,
                                                                        self.ghenv).run()
        except Exception as e:
            self.HuaGong_solver = None
            self.Log.append(u"[Step3] HuaGong solver failed: {}".format(e))
    ...
    def step4_jiaohudou_sandou_and_align(self):
        ...
        try:
            hg_edge_mid = getattr(self.HuaGong_solver, "EdgeMidPoints",
                                  None) if self.HuaGong_solver is not None else None
            hg_corner0 = getattr(self.HuaGong_solver, "Corner0Planes",
                                 None) if self.HuaGong_solver is not None else None
        ...
        try:
            sp_idx = _as_int(self.All_dict.get("VSG3_GA_JiaoHuDou__SourcePlane", 0), 0)
            src_plane0 = _pick_by_index(self.JiaoHuDou_FacePlaneList, sp_idx, None)

            # TargetPlane：PlaneFromLists::3-1 的 ResultPlane（可能为单值或列表）
            tgt_plane_in = self.PFL3_1_ResultPlane

            # 读取参数（可能为单值或列表），按 GH 广播到同一长度后逐项循环 align
            rotate_in = self.All_dict.get("VSG3_GA_JiaoHuDou__RotateDeg", 0.0)
            flipx_in = self.All_dict.get("VSG3_GA_JiaoHuDou__FlipX", 0)
            flipy_in = self.All_dict.get("VSG3_GA_JiaoHuDou__FlipY", 0)
            flipz_in = self.All_dict.get("VSG3_GA_JiaoHuDou__FlipZ", 0)
            movex_in = self.All_dict.get("VSG3_GA_JiaoHuDou__MoveX", 0.0)
            movey_in = self.All_dict.get("VSG3_GA_JiaoHuDou__MoveY", 0.0)
            movez_in = self.All_dict.get("VSG3_GA_JiaoHuDou__MoveZ", 0.0)

            geo_in = self.JiaoHuDou_CutTimbers

            geo_list = _ensure_list(geo_in)
            tgt_list = _ensure_list(tgt_plane_in)

            # 以 Geo 和 TargetPlane 的最大长度作为广播长度
            ...
            # 输出：n==1 则标量；否则列表（GH 会识别）
            self.VSG3_JiaoHuDou_SourceOut = so_list[0] if n == 1 else so_list
            self.VSG3_JiaoHuDou_TargetOut = to_list[0] if n == 1 else to_list
            self.VSG3_JiaoHuDou_XFormRaw = x_list[0] if n == 1 else x_list
            self.VSG3_JiaoHuDou_TransformOut = (ght.GH_Transform(x_list[0]) if (n == 1 and x_list[0] is not None)
                                                else [ght.GH_Transform(x) if x is not None else None for x in x_list])
            self.VSG3_JiaoHuDou_MovedGeo = mg_list[0] if n == 1 else mg_list

            self.Log.append(u"[Step4] VSG3_JiaoHuDou aligned (loop). SP_idx={} n={}".format(sp_idx, n))
        except Exception as e:
    ...
    def step5_bineimangong_shuatou_linggong_and_align(self):
        ...
        try:
            ...
            # TransformOut（来自 VSG3_JiaoHuDou）可能是：None / 单个 xform / list[xform] / tree-like
            x3 = self.VSG3_JiaoHuDou_XFormRaw
            if x3 is None:
                x3_tree = [None]
            elif isinstance(x3, (list, tuple)):
                # 如果是嵌套，则视为 tree；否则单分支
                if len(x3) > 0 and any(isinstance(it, (list, tuple)) for it in x3):
                    x3_tree = x3
                else:
                    x3_tree = [x3]
            else:
            ...
            # 把 JiaoHuDou FacePlaneList 作为 Geometry（列表），对每个分支的 xform 列表进行变换
            jhd_planes = _ensure_list(self.JiaoHuDou_FacePlaneList)
            target_plane_tree = []
            for br in x3_tree:
                # br 可能是单个 xform 或 xform 列表
                br_list = br if isinstance(br, (list, tuple)) else [br]
                br_out = []
                for xf in br_list:
                    planes_x = _transform_planes(jhd_planes, xf) if (jhd_planes and xf is not None) else list(
                        jhd_planes)
                    # tp_idx 可广播
                    if isinstance(tp_idx_list, list):
                        idx_here = tp_idx_list[0] if len(tp_idx_list) > 0 else 0
                    else:
                        idx_here = tp_idx_list
...
if __name__ == "__main__":
    ...
    solver = SiPU_INOUT_1ChaoJuantouComponentAssemblySolver(
        DBPath,
        PlacePlane,
        Refresh,
        ghenv,
