...
def _xform_planes(planes, xform):
    """把 Transform 应用到 plane list；支持单 plane 或 plane list。"""
    if xform is None:
        return planes
    pls = _ensure_list(planes)
    out = []
    for p in pls:
        if p is None:
            out.append(None)
...
def _pfl_broadcast(origin_points, base_planes, index_origin, index_plane, wrap=True):
    """对 FTPlaneFromLists.build_plane 做广播封装。"""
    builder = FTPlaneFromLists(wrap=wrap)

    origin_points = _ensure_list(origin_points)
    base_planes = _ensure_list(base_planes)

    io = index_origin if isinstance(index_origin, (list, tuple)) else [index_origin]
    ip = index_plane if isinstance(index_plane, (list, tuple)) else [index_plane]

    rows = _broadcast_lists(io, ip)
    base_out, origin_out, result_out, log_out = [], [], [], []
    ...
    for (i_o, i_p) in rows:
        ...
        base_out.append(bp)
        origin_out.append(op)
        result_out.append(rp)
        log_out.append(lg)
...
class ChongGongComponentAssemblySolver(object):
    ...
    def step2_jiaohudou(self):
        ...
        self.VSG1_TransformOut = ght.GH_Transform(xf) if xf is not None else None
    ...
    def step3_guazigong(self):
        ...
        self.VSG2_TransformOut = ght.GH_Transform(xf) if xf is not None else None
    ...
    def step5_mangong(self):
        ...
        self.VSG4_TransformOut = ght.GH_Transform(xf) if xf is not None else None
...
# -------------------------------
    ...
    def step6_sandou_qixindou_2(self):
        ...
        self.VSG5_SD_TransformOut = [ght.GH_Transform(xf) if xf is not None else None for xf in xf_list]
        ...
        self.VSG5_QX_TransformOut = ght.GH_Transform(xf) if xf is not None else None
    ...
    def step7_sufang_luohan(self):
        ...
        self.VSG6_TransformOut = ght.GH_Transform(xf) if xf is not None else None
...
if __name__ == "__main__":
    ...
    solver = ChongGongComponentAssemblySolver(
        DBPath=_db,
        PlacePlane=_pp,
        Refresh=_rf,
