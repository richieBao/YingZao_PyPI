...
输入：
    DBPath      : SQLite 数据库路径
    PlacePlane  : 主木坯放置参考输入；若为 Plane，则其原点作为 base_point，平面本身作为 reference_plane；
                  若为 Point/Point3d，则该点作为 base_point，reference_plane 仍从数据库读取
    Table       : 数据表名，默认 "DouGong"
    KeyField    : 主键字段名，默认 "type_code"
    KeyValue    : 主键值，默认 "LU_DOU"
    Field       : JSON 字段名，默认 "params_json"
...
if 'KeyField' not in globals() or not KeyField:
    KeyField = "type_code"

if 'KeyValue' not in globals() or not KeyValue:
    KeyValue = "LU_DOU"

if 'Field' not in globals() or not Field:
    Field = "params_json"
...
def _unwrap_gh_value(val):
    val = _coerce_first(val)
    if val is None:
        return None
    ...
    if hasattr(val, "ScriptVariable"):
        try:
            inner = val.ScriptVariable()
            if inner is not None:
                return inner
        except Exception:
            pass
...
def _coerce_place_input(val):
    val = _unwrap_gh_value(val)

    if isinstance(val, System.Guid):
        obj = sc.doc.Objects.FindId(val)
        if obj is not None:
            val = getattr(obj, "Geometry", None) or obj
    ...
    if isinstance(val, rg.Point):
        return ("point", rg.Point3d(val.Location))

    if isinstance(val, rg.Point3d):
        return ("point", rg.Point3d(val))

    location = getattr(val, "Location", None)
    if isinstance(location, rg.Point3d):
        return ("point", rg.Point3d(location))
    if isinstance(location, rg.Point):
        return ("point", rg.Point3d(location.Location))

    point = getattr(val, "Point", None)
    if isinstance(point, rg.Point3d):
        return ("point", rg.Point3d(point))
    if isinstance(point, rg.Point):
        return ("point", rg.Point3d(point.Location))

    origin = getattr(val, "Origin", None)
    if isinstance(origin, rg.Point3d):
        return ("point", rg.Point3d(origin))
...
class LUDouSolver(object):
    """
    把櫨枓整套流水线封装为一个类：
        solver = LUDouSolver(DBPath, PlacePlane, ghenv, Table, KeyField, KeyValue, Field)
        solver.run()
    然后从 solver.xxx 取任意中间/最终结果作为输出端。
    """

    def __init__(self, db_path, place_plane, ghenv,
                 table="DouGong", key_field="type_code",
                 key_value="LU_DOU", field="params_json"):
        self.db_path = db_path
        self.place_plane_input = place_plane
        self.ghenv = ghenv
        self.table = table or "DouGong"
        self.key_field = key_field or "type_code"
        self.key_value = key_value or "LU_DOU"
        self.field = field or "params_json"
    ...
    def _input_has_source(self, input_name):
        try:
            if self.ghenv is None or self.ghenv.Component is None:
                return False

            params = self.ghenv.Component.Params.Input
            for param in params:
                if param is None:
                    continue

                if param.Name == input_name or param.NickName == input_name:
                    sources = getattr(param, "Sources", None)
    ...
    def step_db(self):
        resolved_db_path, db_path_source = resolve_db_path(self.db_path, ghenv=self.ghenv)
        if not resolved_db_path:
            self.DBLog.append(u"错误：DBPath 为空，请提供数据库路径。")
            return

        self.db_path = resolved_db_path
        if db_path_source == "document_default":
            self.DBLog.append(u"DBPath 未显式输入，已回退到当前 GH 文档默认 DBPath。")

        JsonPath  = ""
        ExportAll = True

        self.DBLog.append(
            u"数据库查询参数：Table='{}', KeyField='{}', KeyValue='{}', Field='{}'。".format(
                self.table, self.key_field, self.key_value, self.field
            )
        )
    ...
    def step_main_timber(self):
        ...
        place_plane_connected = self._input_has_source("PlacePlane")
        place_kind = None
        place_value = None
        if not place_plane_connected:
            log.append(u"PlacePlane 输入端未连接，reference_plane 使用 DB/默认值。")
        else:
            place_kind, place_value = _coerce_place_input(self.place_plane_input)

        if place_kind is None and place_plane_connected and self.place_plane_input is not None:
            log.append(u"提示：PlacePlane 输入类型为 {}，已忽略，由 DB/默认决定。"
                       .format(type(self.place_plane_input)))

        def _get_base_point_for_block(dct, full_key, place_plane, place_point, log_list):
            if isinstance(place_plane, rg.Plane) and place_plane.IsValid:
                pt = rg.Point3d(place_plane.Origin)
                log_list.append(
                    u"base_point 采用 PlacePlane 原点：({:.3f}, {:.3f}, {:.3f})。"
                    .format(pt.X, pt.Y, pt.Z)
                )
                return pt
            if isinstance(place_point, rg.Point3d):
                log_list.append(
                    u"base_point 采用 PlacePlane 点输入：({:.3f}, {:.3f}, {:.3f})。"
                    .format(place_point.X, place_point.Y, place_point.Z)
                )
                return rg.Point3d(place_point)
            return self._get_point_from_dict(dct, full_key, rg.Point3d(0, 0, 0), log_list)

        def _get_reference_plane_for_block(dct, full_key, place_plane, log_list):
            if isinstance(place_plane, rg.Plane) and place_plane.IsValid:
                log_list.append(u"reference_plane 采用 PlacePlane 输入值。")
                return rg.Plane(place_plane)
            return self._get_plane_from_dict(dct, full_key, log_list)
        ...
        base_point_final      = _get_base_point_for_block(
            d,
            PREFIX_BLOCK + "base_point",
            place_value if place_kind == "plane" else None,
            place_value if place_kind == "point" else None,
            log
        )
        reference_plane_final = _get_reference_plane_for_block(
            d,
            PREFIX_BLOCK + "reference_plane",
            place_value if place_kind == "plane" else None,
            log
        )
    ...
    def step_align1(self):
        ...
        try:
            block_rot_deg_list = self._get_number_list_from_dict(
                d, PREFIX_ALIGN1 + "BlockRotDeg", log, default=0.0
            )
            ...
            block_rot_deg_list = _broadcast(block_rot_deg_list, count, 0.0)
            flip_y_list = _broadcast(flip_y_list, count, False)

            log.append(u"FT_AlignToolToTimber::1 将执行 {} 组对位。".format(count))
            log.append(
                u"广播策略：ToolGeo/ToolBasePlane 使用单值；BlockFacePlane/BlockRotDeg/FlipY 按 {} 组广播。"
                .format(count)
            )

            for i in range(count):
                block_plane = self.ResultPlane1[i]
                block_rot_deg = block_rot_deg_list[i]
                flip_y = flip_y_list[i]
