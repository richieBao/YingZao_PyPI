# -*- coding: utf-8 -*-
"""
QiAngDouSolver —— 騎昂枓一体化求解器 (Step 1 + Step 2 + Step 3 + Step 4)

步骤说明：
    Step 1: DBJsonReader 读取 DG_Dou 表中 QiAng_DOU 的 params_json
...
class QiAngDouSolver(object):
    ...
    def __init__(self, DBPath, base_point, Refresh):
        ...
        # Align::3（GeoAligner_xfm）
        self.SourceOut3 = []
        self.TargetOut3 = []
        self.TransformOut3 = []
        self.AlignedTool3 = []  # 实际为 MovedGeo（楔形刀对位后）
    ...
    def step4_block_cutter(self):
        ...
        # ----------------- 4.5 预备刀具列表：QiAo + BlockCutter（不在此处切削） -----------------
        tools_all = []
        ...
        self.ToolsAll = tools_all
        self.Log.append("[Tools] Step4 收集刀具数量 = {}".format(len(self.ToolsAll)))
    ...
    # -----------------------------------------------------
    # Step 5：楔形刀切削 —— XieXingTool + PFL4 + Align::3 + Cut(V3)
    # -----------------------------------------------------
    def step5_wedge_and_cut(self):

        self.Log.append("===== Step 5: XieXingTool + PFL4 + Align::3 + Cut(V3) =====")

        if self.AllDict is None:
            self.Log.append("[Step 5] AllDict 为空，无法读取参数。")
            return

        # ----------------- 5.1 XieXingTool -----------------
        try:
            # base_point 默认原点（按组件约定）
            tool_bp = rg.Point3d(0, 0, 0)

            # 参考平面：沿用 GH XZ 语义（WorldXZ）
            tool_ref_plane = "WorldXZ"

            AB_raw = all_get(self.AllDict, "XieXingTool", "AB", 6.0)
            AC_raw = all_get(self.AllDict, "XieXingTool", "AC", 14.0)
            offset_raw = all_get(self.AllDict, "XieXingTool", "offset", 3.0)

            AB = float(to_scalar(AB_raw, 6.0))
            AC = float(to_scalar(AC_raw, 14.0))
            offset = float(to_scalar(offset_raw, 3.0))

            tool = WedgeShapedTool(
                base_point=tool_bp,
                reference_plane=tool_ref_plane,
                AB=AB,
                AC=AC,
                offset=offset
            )
            ...
            self.XieXing_SectionCrv = tool.section_crv
            self.XieXing_PlaneOut = tool.plane_out
            self.XieXing_A_RefPlanes = tool.a_ref_planes
            self.XieXing_Log = tool.get_log()

            if self.XieXing_Log:
                for l in self.XieXing_Log:
                    self.Log.append("[XieXingTool] " + str(l))

            if self.XieXing_SolidBrep is None:
                self.Log.append("[XieXingTool] SolidBrep 为空，跳过后续对位与切削。")
                return

        except Exception as e:
            self.Log.append("[XieXingTool] 构建失败: {}".format(e))
            self.XieXing_SolidBrep = None
            self.XieXing_A_RefPlanes = None
            return

        # ----------------- 5.2 PlaneFromLists::4 -----------------
        idx4_o_raw = all_get(self.AllDict, "PlaneFromLists_4", "IndexOrigin", [])
        idx4_p_raw = all_get(self.AllDict, "PlaneFromLists_4", "IndexPlane", [])
        wrap4_raw  = all_get(self.AllDict, "PlaneFromLists_4", "wrap", True)

        idx4_o_list = _to_list(idx4_o_raw)
        idx4_p_list = _to_list(idx4_p_raw)
        n_pf4 = min(len(idx4_o_list), len(idx4_p_list))
        ...
        if n_pf4 == 0:
            self.Log.append("[PFL4] IndexOrigin / IndexPlane 列表为空，跳过 PlaneFromLists::4。")
        else:
            wrap4 = bool(_scalar_from_list(wrap4_raw, True))
            builder4 = FTPlaneFromLists(wrap=wrap4)

            for i in range(n_pf4):
                io4 = idx4_o_list[i]
                ip4 = idx4_p_list[i]
                self.Log.append("[PFL4] 第 {} 组：IndexOrigin = {}, IndexPlane = {}".format(i, io4, ip4))

                try:
                    bp4, op4, rp4, log_pf4 = builder4.build_plane(
                        self.EdgeMidPoints,
                        self.Corner0Planes,
                        io4,
                        ip4
                    ...
                    self.BasePlane4.append(bp4)
                    self.OriginPoint4.append(op4)
                    self.ResultPlane4.append(rp4)

                    for l in log_pf4:
                        self.Log.append("[PFL4][{}] ".format(i) + l)

                except Exception as e:
                    self.Log.append("[PFL4] 第 {} 组 build_plane 出错: {}".format(i, e))

        # ----------------- 5.3 Align::3（GeoAligner_xfm）-----------------
        # Geo 为楔形刀 SolidBrep；SourcePlane 为 A_RefPlanes（按索引取）；TargetPlane 为 ResultPlane4
        src_plane_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "SourcePlane", 0)
        flipx_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "FlipX", 0)

        RotateDeg_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "RotateDeg", 0.0)
        FlipY_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "FlipY", 0)
        FlipZ_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "FlipZ", 0)
        MoveX_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "MoveX", 0.0)
        MoveY_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "MoveY", 0.0)
        MoveZ_raw = all_get(self.AllDict, "FT_AlignToolToTimber_3", "MoveZ", 0.0)

        # 规范化 SourcePlane 索引
        try:
            src_idx = int(to_scalar(src_plane_raw, 0))
        except Exception:
            src_idx = 0

        # 从 A_RefPlanes 取 plane
        src_planes = _to_list(self.XieXing_A_RefPlanes)
        if not src_planes:
            self.Log.append("[Align3] A_RefPlanes 为空，无法对位。")
            self.AlignedTool3 = []
        else:
            if src_idx < 0 or src_idx >= len(src_planes):
                self.Log.append("[Align3] SourcePlane 索引超界: {}，将使用 0。".format(src_idx))
                src_idx = 0

            src_plane = src_planes[src_idx]

            tgt_planes = _to_list(self.ResultPlane4)
            if not tgt_planes:
                self.Log.append("[Align3] TargetPlane(ResultPlane4) 为空，无法对位。")
                self.AlignedTool3 = []
            else:
                # 广播：一个楔形刀对多平面时，按目标平面数量重复对位
                N3 = len(tgt_planes)

                flipx_list = _broadcast_param(flipx_raw, N3, "FlipX3")
                flipy_list = _broadcast_param(FlipY_raw, N3, "FlipY3")
                flipz_list = _broadcast_param(FlipZ_raw, N3, "FlipZ3")
                rot_list   = _broadcast_param(RotateDeg_raw, N3, "RotateDeg3")
                mx_list    = _broadcast_param(MoveX_raw, N3, "MoveX3")
                my_list    = _broadcast_param(MoveY_raw, N3, "MoveY3")
                mz_list    = _broadcast_param(MoveZ_raw, N3, "MoveZ3")
                ...
                for i in range(N3):
                    try:
                        SourceOut, TargetOut, TransformOut, MovedGeo = GeoAligner_xfm.align(
                            self.XieXing_SolidBrep,
                            src_plane,
                            tgt_planes[i],
                            rotate_deg=float(to_scalar(rot_list[i], 0.0)),
                            flip_x=int(to_scalar(flipx_list[i], 0)),
                            flip_y=int(to_scalar(flipy_list[i], 0)),
                            flip_z=int(to_scalar(flipz_list[i], 0)),
                            move_x=float(to_scalar(mx_list[i], 0.0)),
                            move_y=float(to_scalar(my_list[i], 0.0)),
                            move_z=float(to_scalar(mz_list[i], 0.0)),
                        )

                        self.SourceOut3.append(SourceOut)
                        self.TargetOut3.append(TargetOut)
                        self.TransformOut3.append(TransformOut)
                        self.AlignedTool3.append(MovedGeo)

                    except Exception as e:
                        self.Log.append("[Align3] 第 {} 组对位失败: {}".format(i, e))
                        self.SourceOut3.append(None)
                        self.TargetOut3.append(None)
                        self.TransformOut3.append(None)
                        self.AlignedTool3.append(None)

        # ----------------- 5.4 最终切削：使用 FT_CutTimberByTools-V3 -----------------
        tools_all = []
        if self.ToolsAll:
            tools_all.extend([t for t in self.ToolsAll if t is not None])
        if self.AlignedTool3:
            tools_all.extend([t for t in self.AlignedTool3 if t is not None])

        if self.TimberBrep is None or not tools_all:
            self.Log.append("[CutV3] TimberBrep 或 Tools 为空，无法切削。")
            self.CutTimbers = []
            self.FailTimbers = []
            return

        try:
            # V3 cutter：GH SolidDifference 实现
            cutter = FT_CutTimbersByTools_GH_SolidDifference(debug=False)
            cut, fail, logc = cutter.cut(
            ...
            for l in logc:
                self.Log.append("[CutV3] " + str(l))

        except Exception as e:
            self.Log.append("[CutV3] 切削失败: {}".format(e))
            self.CutTimbers = []
            self.FailTimbers = []
...
if __name__ == "__main__":
    ...
    solver = QiAngDouSolver(DBPath, base_point, Refresh)
    solver.run()
    ...
    BasePlane4   = solver.BasePlane4
    OriginPoint4 = solver.OriginPoint4
    ResultPlane4 = solver.ResultPlane4

    SourceOut3     = solver.SourceOut3
    TargetOut3     = solver.TargetOut3
    TransformOut3  = solver.TransformOut3
