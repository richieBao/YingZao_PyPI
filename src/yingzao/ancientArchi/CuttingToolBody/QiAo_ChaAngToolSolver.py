# -*- coding: utf-8 -*-
"""
QiAo_ChaAngToolSolver.py (Refactored + MultiCut)

乳栿劄牽_欹䫜刀（QiAo）·宋 · 单一 GhPython 组件（不使用数据库）
并内置“叉昂（ChaAng）”二次裁切：
  - 第一次：用欹䫜刀切削 Timber
  - 第二次：在 width 方向中点 A 建立平面 P_A，绘制截面 A-C-E-A，偏移±offset 成体，再对第一次结果二次差集
------------------------------------------------------------
本版本解决：
...
height_fen : float        Access:item  TypeHint:float   默认 6  （MultiCut：ChaAng 截面高度默认 6）
...
# --- ChaAng（二次裁切）---
AE_length : float Access:item TypeHint:float 默认 14
offset    : float Access:item TypeHint:float 默认 4
...
__version__ = "2026.01.09-qiao-chaang-multicut-v1"
...
# =========================================================
# Solver：QiAo_ChaAngToolSolver
# =========================================================

class QiAo_ChaAngToolSolver(object):
    ...
    def __init__(self, ghenv=None):
        ...
        # 第一次（欹䫜）裁切结果
        self.CutTimbers_QiAo = []
        self.FailTimbers_QiAo = []

        # 第二次（ChaAng）裁切结果（最终输出）
        self.CutTimbers = []
        self.FailTimbers = []
    ...
    def step5_cut(self):
        try:
            ...
            if ghc is not None:
                ...
                self.CutTimbers_QiAo = cut
                self.FailTimbers_QiAo = fail
                self.Log.append("[CUT-QIAO] OK(GH): CutTimbers={}, FailTimbers={}".format(len(InputHelper.flatten(cut)), len(InputHelper.flatten(fail))))
                self.Log.extend(InputHelper.flatten(log_lines))
                return self
            ...
            self.CutTimbers_QiAo = cut
            self.FailTimbers_QiAo = fail if fail is not None else []
            self.Log.append("[CUT-QIAO] OK(Fallback)")
            self.Log.extend(InputHelper.flatten(log_lines))
        except Exception as e:
            self.CutTimbers_QiAo = []
            self.FailTimbers_QiAo = []
            self.Log.append("[CUT-QIAO][ERROR] {}".format(e))
        return self

    # ----------------------------
    # Step6 ChaAng cutter (A-C-E-A section @ P_A, then ±offset)
    # ----------------------------
    def step6_build_chaang_cutter(self, params):
        """构建 ChaAng 裁切体。

        规则：
        - width_dir = timber_ref_plane.Normal（垂直参考平面的方向）
        - 点 A = base_point 沿 width_dir 偏移 width_fen/2 的中点
        - 平面 P_A：轴系同 timber_ref_plane，但 Origin = A
        - E = A + XAxis * AE_length
        - C = A + YAxis * height_fen （此处 height_fen 默认 6，用于截面高度）
        - 截面 A-C-E-A 偏移±offset：等价为沿 P_A.Normal 挤出 2*offset 后回移 -offset
        """
        self.ChaAngPlane_PA = None
        self.PtA = self.PtC = self.PtE = None
        self.ChaAngSectionCurve = None
        self.ChaAngCutterBrep = None

        try:
            bp = params["base_point"]
            timber_pl = params["timber_ref_plane"]
            w = float(params["width_fen"])
            h = float(params["height_fen"])
            AE = float(params.get("AE_length", 14.0))
            off = float(params.get("offset", 4.0))

            if timber_pl is None:
                self.Log.append("[CHAANG][SKIP] timber_ref_plane is None")
                return self

            width_dir = rg.Vector3d(timber_pl.Normal)
            if width_dir.IsZero:
                self.Log.append("[CHAANG][ERROR] timber_ref_plane.Normal is zero")
                return self
            width_dir.Unitize()

            A = rg.Point3d(bp) + width_dir * (w * 0.5)
            P_A = rg.Plane(timber_pl)
            P_A.Origin = A

            # Points on section plane
            E = A + rg.Vector3d(P_A.XAxis) * AE
            C = A + rg.Vector3d(P_A.YAxis) * h

            # Section curve
            section_crv = rg.Polyline([A, C, E, A]).ToNurbsCurve()

            # Build cutter brep: extrude along plane normal (±offset)
            n = rg.Vector3d(P_A.Normal)
            if n.IsZero:
                self.Log.append("[CHAANG][ERROR] P_A.Normal is zero")
                return self
            n.Unitize()
            extrude_vec = n * (off * 2.0)

            # RhinoCommon 在不同版本/运行时（Rhino/Grasshopper IronPython）中，
            # `Brep.CreateFromExtrusion` 可能不可用；用更通用的 Surface.CreateExtrusion 替代。
            # 先生成侧向挤出曲面，再 ToBrep + 端面封口，得到可用于差集的封闭体。
            srf = rg.Surface.CreateExtrusion(section_crv, extrude_vec)
            if srf is None:
                self.Log.append("[CHAANG][ERROR] Surface.CreateExtrusion returned None")
                return self

            cutter_brep = srf.ToBrep()
            if cutter_brep is None:
                self.Log.append("[CHAANG][ERROR] Surface.ToBrep returned None")
                return self

            # 封口（端面）
            tol = getattr(getattr(sc, "doc", None), "ModelAbsoluteTolerance", 0.01) or 0.01
            capped = cutter_brep.CapPlanarHoles(tol)
            if capped is not None:
                cutter_brep = capped

            # center it to realize ±offset
            cutter_brep.Transform(rg.Transform.Translation(-n * off))

            self.ChaAngPlane_PA = P_A
            self.PtA, self.PtC, self.PtE = A, C, E
            self.ChaAngSectionCurve = section_crv
            self.ChaAngCutterBrep = cutter_brep
            self.Log.append("[CHAANG] OK: AE_length={}, offset={}".format(AE, off))
        except Exception as e:
            self.ChaAngCutterBrep = None
    ...
    # ----------------------------
    # Step7 Cut ChaAng (second pass)
    # ----------------------------
    def _boolean_diff_fallback(self, timbers, tools):
        """RhinoCommon 兜底差集：逐件 BooleanDifference。"""
        timber_list = [b for b in InputHelper.flatten(timbers) if isinstance(b, rg.Brep)]
        tool_list = [b for b in InputHelper.flatten(tools) if isinstance(b, rg.Brep)]
        tol = getattr(getattr(sc, "doc", None), "ModelAbsoluteTolerance", 0.01) or 0.01
        out, fail = [], []
        for i, tb in enumerate(timber_list):
            try:
                res = rg.Brep.CreateBooleanDifference(tb, tool_list, tol)
    ...
    def step7_cut_chaang(self):
        try:
            base_cut = InputHelper.flatten(self.CutTimbers_QiAo)
            tool = self.ChaAngCutterBrep

            if not base_cut:
                self.CutTimbers = []
                self.FailTimbers = []
                self.Log.append("[CUT-CHAANG][SKIP] CutTimbers_QiAo is empty")
                return self

            if tool is None:
                # 允许仅输出第一次结果
                self.CutTimbers = base_cut
                self.FailTimbers = InputHelper.flatten(self.FailTimbers_QiAo)
                self.Log.append("[CUT-CHAANG][SKIP] ChaAngCutterBrep is None -> output QIAO result")
                return self

            if ghc is not None:
                cutter = GHSolidDifferenceCutter(debug=False)
                cut, fail, log_lines = cutter.cut(base_cut, [tool])
                self.CutTimbers = cut
                self.FailTimbers = fail
                self.Log.append("[CUT-CHAANG] OK(GH): CutTimbers={}, FailTimbers={}".format(len(InputHelper.flatten(cut)), len(InputHelper.flatten(fail))))
                self.Log.extend(InputHelper.flatten(log_lines))
                return self

            cut, fail = self._boolean_diff_fallback(base_cut, [tool])
            self.CutTimbers = cut
            self.FailTimbers = fail
            self.Log.append("[CUT-CHAANG] OK(Fallback)")
        except Exception as e:
            self.CutTimbers = InputHelper.flatten(self.CutTimbers_QiAo)
            self.FailTimbers = InputHelper.flatten(self.FailTimbers_QiAo)
            self.Log.append("[CUT-CHAANG][ERROR] {}".format(e))
        return self
    ...
    def run(self, params):
        ...
        self.step5_cut()
        # ChaAng second pass
        self.step6_build_chaang_cutter(params)
        self.step7_cut_chaang()
        return self
...
def _ensure_all_inputs_optional_once():
    try:
        ...
        if comp is None:
            return
        key = "QiAo_ChaAngToolSolver::inputs_optional_set::{}".format(__version__)
        if key in sc.sticky:
            return
...
if __name__ == "__main__":
    ...
    params = {
        ...
        # MultiCut：ChaAng 截面高度默认 6（也作为 Timber 高度默认值，避免未接线时与截面不一致）
        "height_fen": _to_float(_height_fen, 6.0),
        "base_point": bp,
        ...
        # ChaAng（二次裁切）
        "AE_length": _to_float(_AE_length, 14.0),
        "offset": _to_float(_offset, 4.0),
    }
    ...
    # 3) solver（sticky 可选）
    # -------------------------
    sticky_key = "QiAo_ChaAngToolSolver::{ver}".format(ver=__version__)
    ...
    if solver is None:
        solver = QiAo_ChaAngToolSolver(ghenv=ghenv)
