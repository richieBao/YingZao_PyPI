# -*- coding: utf-8 -*-
"""
GHpy :: TimberSection_ABFE_SchemeD  (+ Qin / PiZhu switch)
------------------------------------------------------------
方案 D（按用户要求）：
- 约束1：E_L 过 A
- 约束2：E_U 过 D
...
OD              : float        Access:item  TypeHint:float     默认 36.0   （O->D 的 H 向距离，即 u_D）
...
【可选对照输出（不参与约束，仅用于参考）】
OB, BC          : float        Access:item  TypeHint:float     默认 22.0, 14.0

============================================================
【GhPython 输出 Outputs（建议 GH 端配置）】
O,A,B,D,E,F,G           : rg.Point3d   Access:item  TypeHint:Point3d
...
class ChaAngQiAo(object):
    def __init__(self, base_point_, ref_plane_mode_="WorldXZ",
        ...
        # base outputs
        self.O = self.A = self.B = self.D = None
    ...
    def solve(self):
        ...
        # D 直接给定：u=OD, v=-CD
        self.D = self._pt_uv(self.OD, -self.CD)
        ...
        for nx, ny, alpha in n_candidates:
            du, dv = self._perp_ccw(nx, ny)
            d_candidates.append((du, dv, nx, ny, alpha))
            d_candidates.append((-du, -dv, nx, ny, alpha))
        ...
        # 选与 d 垂直的一组 n，并优先让 shift 的 v 分量为负
        best = None
        best_penalty = 1e99
        for du0, dv0, nx, ny, alpha in d_candidates:
            if (du0 * du + dv0 * dv) < 0.999:
                continue
            if abs(nx * du + ny * dv) > 1e-6:
                continue
            s_signed = alpha * dist_AD
            shift_v = ny * s_signed
            penalty = 0.0 if shift_v < 0 else 10.0
        ...
        # 构造 EU / EL（长线用于显示/求交）
        L = 1e4
        EU_p0 = self._pt_uv(self.OD - du * L, -self.CD - dv * L)
        EU_p1 = self._pt_uv(self.OD + du * L, -self.CD + dv * L)
        ...
        self.log.append("Info: Given (OB+BC) = {:.6f} (reference only)".format(float(OB) + float(BC)))
